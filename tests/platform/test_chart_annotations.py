...
def _root(path: Path) -> ProductionCompositionRoot:
    ...
    if connection.execute("SELECT count(*) FROM data_snapshot WHERE data_snapshot_id='snapshot_chart'").fetchone()[0] == 0:
        with connection:
            ...
            connection.execute("INSERT INTO normalized_record VALUES(?,?,?)", ("record_chart", "daily", "security_yihua:2026-07-10"))
            connection.execute("INSERT INTO normalized_version VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", ("version_chart", "record_chart", 1, "chart-content", "attempt_chart", "2026-07-10", "2026-07-10T08:00:00+00:00", "timestamp", "2026-07-10T08:00:00+00:00", "publisher_timestamp", "2026-07-10T09:00:00+00:00", "pass", None))
            connection.execute("INSERT INTO ohlcv_version VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", ("version_chart", "security_yihua", "2026-07-10", "Asia/Shanghai", "none", "88.51", "91.00", "82.33", "82.33", "221879.03", "hand", "1926373.75544", "thousand_cny", "CNY"))
            connection.execute("INSERT INTO data_snapshot VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", ("snapshot_chart", "security_yihua", "chart", "2026-07-11", "2026-07-10", "2026-07-11T00:00:00+00:00", "Asia/Shanghai", "cn-calendar@2026", "chart-query@1", "source@1", "freshness@1", "chart-members", "valid", "pass", 1, 1, 0, 0, 1, "effective_complete_session", "2026-07-10T09:00:00+00:00"))
            connection.execute("INSERT INTO data_snapshot_member VALUES(?,?,?,?)", ("snapshot_chart", "version_chart", "daily", 0))
    return root


def _draft(price: str = "82.3300") -> AnnotationDraft:
    return AnnotationDraft(
        "security_yihua", "1d", "none", "snapshot_chart", None, "horizontal_line", "accent", "local-user",
...
def test_local_workspace_http_reload_and_server_restart_restore_sqlite_state(tmp_path: Path) -> None:
    ...
    payload = json.dumps({"kind": "trend_line", "style": "accent", "anchors": [{"market_timestamp": "2026-07-10T15:00:00+08:00", "exact_price_decimal": "82.3300"}]}).encode()
    request = Request(base + "/api/annotations", data=payload, method="POST", headers={"Content-Type": "application/json", "Origin": base, "X-CSRF-Token": token, "X-Invocation-Id": "browser:create"})
    ...
    def command(invocation: str, body: dict[str, object]) -> dict[str, object]:
        ...
        call = Request(base + "/api/annotations", data=encoded, method="POST", headers={"Content-Type": "application/json", "Origin": base, "X-CSRF-Token": token, "X-Invocation-Id": invocation})
        return json.loads(urlopen(call).read())
    revised = command("browser:revise", {"operation": "revise", "annotation_id": created["annotation_id"], "expected_version_no": 1, "kind": "trend_line", "style": "warning", "anchors": [{"market_timestamp": "2026-07-10T15:00:00+08:00", "exact_price_decimal": "83.1250"}]})
    deleted = command("browser:delete", {"operation": "delete", "annotation_id": created["annotation_id"], "expected_version_no": 2})
    restored_version = command("browser:restore", {"operation": "restore", "annotation_id": created["annotation_id"], "expected_version_no": 3})
    assert [revised["version_no"], deleted["version_no"], restored_version["version_no"]] == [2, 3, 4]
    ...
    restarted.close(); rebuilt.close()


@pytest.mark.parametrize("draft", [
    replace(_draft(), kind="pixel"),
    replace(_draft(), adjustment_mode="mystery"),
    replace(_draft(), adjustment_mode="forward", factor_snapshot_id="snapshot_chart"),
    replace(_draft(), anchors=(AnnotationAnchor("not-a-market-time", "82"),)),
    replace(_draft(), anchors=(AnnotationAnchor("garbageTvalue", "82"),)),
    replace(_draft(), anchors=(AnnotationAnchor("2026-07-10T15:00:00+08:00", "NaN"),)),
    replace(_draft(), anchors=(AnnotationAnchor("2026-07-10T15:00:00+08:00", "1e100000000"),)),
    replace(_draft(), anchors=(AnnotationAnchor("2026-07-10T15:00:00+08:00", "123456789012345678901"),)),
])
