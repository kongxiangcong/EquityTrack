...
def _expire(root: PlatformTaskFixture, run_id: str) -> None:
    with root.faults.adapter_connection:
        root.faults.adapter_connection.execute("UPDATE workflow_run SET lease_expires_at='2000-01-01T00:00:00+00:00' WHERE workflow_run_id=?", (run_id,))
...
def test_resume_reuses_committed_nodes_and_never_duplicates_research(
    ...
    run_id = root.faults.adapter_connection.execute("SELECT workflow_run_id FROM workflow_run WHERE invocation_id='recovery:nodes'").fetchone()[0]
    ...
    attempts = root.faults.adapter_connection.execute("SELECT node_id,count(*) count FROM workflow_node_run JOIN workflow_node_attempt USING(workflow_node_run_id) WHERE workflow_run_id=? GROUP BY node_id", (run_id,)).fetchall()
    assert all(row["count"] <= 2 for row in attempts)
    assert root.faults.workflow_ledger.audit_integrity(IntegrityScope()).errors == ()
    root.close()
...
def test_lease_takeover_abandons_attempt_and_rejects_live_second_owner(tmp_path: Path) -> None:
    ...
    run_id = root.faults.adapter_connection.execute("SELECT workflow_run_id FROM workflow_run WHERE invocation_id='recovery:lease'").fetchone()[0]
    ...
    attempts = root.faults.adapter_connection.execute("SELECT attempt_no,disposition FROM workflow_node_attempt a JOIN workflow_node_run n USING(workflow_node_run_id) WHERE n.workflow_run_id=? AND n.node_id='run_or_link_research' ORDER BY attempt_no", (run_id,)).fetchall()
...
def test_transient_failure_creates_bounded_monotonic_attempts(tmp_path: Path) -> None:
    ...
    attempts = root.faults.adapter_connection.execute("SELECT attempt_no,disposition,retryable FROM workflow_node_attempt a JOIN workflow_node_run n USING(workflow_node_run_id) WHERE n.workflow_run_id=? AND n.node_id='run_or_link_research' ORDER BY attempt_no", (result.workflow_run_id,)).fetchall()
...
def test_nonretryable_failure_is_terminal_after_one_attempt(tmp_path: Path) -> None:
    ...
    attempts = root.faults.adapter_connection.execute("SELECT attempt_no,retryable FROM workflow_node_attempt a JOIN workflow_node_run n USING(workflow_node_run_id) WHERE n.node_id='run_or_link_research'").fetchall()
...
def test_periodic_heartbeat_prevents_takeover_during_slow_engine_call(tmp_path: Path) -> None:
    ...
    with pytest.raises(InjectedCrash):
        root.research.handle(StartResearchWorkflow(request))
    run_id = root.faults.adapter_connection.execute("SELECT workflow_run_id FROM workflow_run LIMIT 1").fetchone()[0]
...
def test_resume_revalidates_quality_and_domain_references(tmp_path: Path, corruption: str, code: str) -> None:
    ...
    if corruption == "quality":
        root.faults.adapter_connection.execute("UPDATE data_snapshot SET quality_status='blocking' WHERE snapshot_purpose='research'")
    else:
        root.faults.adapter_connection.execute("DELETE FROM research_input_projection")
...
def test_resume_fails_closed_on_definition_fingerprint_or_artifact_corruption(tmp_path: Path, corruption: str, code: str) -> None:
    ...
    run_id = root.faults.adapter_connection.execute("SELECT workflow_run_id FROM workflow_run ORDER BY created_at DESC LIMIT 1").fetchone()[0]
    if corruption == "workflow_definition":
        root.faults.adapter_connection.execute("DROP TRIGGER workflow_run_identity_immutable")
        root.faults.adapter_connection.execute("UPDATE workflow_run SET definition_hash='old' WHERE workflow_run_id=?", (run_id,))
    if corruption in {"node_version", "schema", "input_fingerprint"}:
        ...
        root.faults.adapter_connection.execute("DROP TRIGGER workflow_node_contract_immutable")
    if corruption == "node_version":
        root.faults.adapter_connection.execute("UPDATE workflow_node_run SET node_version='old' WHERE workflow_run_id=? AND node_id='freeze_research_projection'", (run_id,))
    elif corruption == "schema":
        root.faults.adapter_connection.execute("UPDATE workflow_node_run SET output_schema='old' WHERE workflow_run_id=? AND node_id='freeze_research_projection'", (run_id,))
    elif corruption == "input_fingerprint":
        root.faults.adapter_connection.execute("UPDATE workflow_node_run SET input_fingerprint='tampered' WHERE workflow_run_id=? AND node_id='freeze_research_projection'", (run_id,))
    elif corruption in {"artifact", "missing_artifact"}:
        artifact = root.faults.adapter_connection.execute("SELECT a.object_sha256,o.relative_path FROM workflow_node_run n JOIN artifact_manifest_member m ON m.artifact_manifest_id=n.checkpoint_manifest_id JOIN artifact a USING(artifact_id) JOIN object_blob o ON o.sha256=a.object_sha256 WHERE n.workflow_run_id=? LIMIT 1", (run_id,)).fetchone()
...
def test_cancellation_and_retry_history_are_monotonic(tmp_path: Path) -> None:
    ...
    assert root.faults.adapter_connection.execute("SELECT status FROM workflow_run WHERE workflow_run_id=?", (run_id,)).fetchone()[0] == "cancelled"
    with pytest.raises(sqlite3.IntegrityError, match="WORKFLOW_TRANSITION_IMMUTABLE"):
        root.faults.adapter_connection.execute("DELETE FROM workflow_transition WHERE workflow_run_id=?", (run_id,))
    root.close()
...
def test_cursor_and_normalized_transaction_roll_back_then_replay_once(tmp_path: Path) -> None:
    ...
    root.faults.workflow_ledger.fault_injector = injector
    root.faults.data_repository.fault_injector = injector
    with pytest.raises(InjectedCrash):
        root.data.sync(sync_request("recovery:cursor"))
    assert root.faults.adapter_connection.execute("SELECT count(*) FROM sync_cursor").fetchone()[0] == 0
    assert root.faults.adapter_connection.execute("SELECT count(*) FROM normalized_version").fetchone()[0] == 0
    root.faults.workflow_ledger.fault_injector = None
    root.faults.data_repository.fault_injector = None
    result = root.data.sync(sync_request("recovery:cursor"))
    assert result.snapshot_id
    assert root.faults.adapter_connection.execute("SELECT count(*) FROM sync_cursor").fetchone()[0] == 3
    version_count = root.faults.adapter_connection.execute("SELECT count(*) FROM normalized_version").fetchone()[0]
    replay = root.data.sync(sync_request("recovery:cursor:replay"))
    assert replay.snapshot_id == result.snapshot_id
    assert root.faults.adapter_connection.execute("SELECT count(*) FROM normalized_version").fetchone()[0] == version_count
    root.close()
...
def test_cursor_response_loss_after_commit_replays_without_duplicate_versions(tmp_path: Path) -> None:
    ...
    committed_cursors = {tuple(row) for row in root.faults.adapter_connection.execute("SELECT provider_id,adapter_version,dataset,scope_id,cursor_value FROM sync_cursor")}
    committed_versions = {row[0] for row in root.faults.adapter_connection.execute("SELECT normalized_version_id FROM normalized_version")}
    root.faults.data_repository.fault_injector = None
    result = root.data.sync(request)
    assert result.snapshot_id
    final_cursors = {tuple(row) for row in root.faults.adapter_connection.execute("SELECT provider_id,adapter_version,dataset,scope_id,cursor_value FROM sync_cursor")}
    final_versions = {row[0] for row in root.faults.adapter_connection.execute("SELECT normalized_version_id FROM normalized_version")}
    assert committed_cursors <= final_cursors and len(final_cursors) == 3
    assert committed_versions <= final_versions
    assert root.faults.adapter_connection.execute("SELECT count(*) FROM normalized_version").fetchone()[0] == len(final_versions)
    root.close()
