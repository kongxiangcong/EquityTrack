...
def test_update_authorization_is_csrf_protected_immutable_and_replay_safe(
    ...
    token = html.split('name="csrf-token" content="', 1)[1].split('"', 1)[0]
    body = json.dumps(
        {"requested_date": "2026-07-11", "effective_session_date": "2026-07-10"}
    ).encode()
    headers = {
        "Origin": base,
        "Content-Type": "application/json",
...
def test_frozen_timeline_traverses_plan_market_evaluation_and_policy_versions(
    tmp_path: Path,
) -> None:
    root, plan = market_root(tmp_path)
    market = root.market.build_market_snapshot(_market_command())
    evaluation = root.market.evaluate_plan(
        EvaluatePlanCommand(
            "workspace:evaluate",
    ...
    upgraded = root.market.evaluate_plan(
        EvaluatePlanCommand(
            "workspace:evaluate:v2",
            plan.plan_version_id,
            market.market_snapshot_id,
            "plan-evaluator@1",
            "evaluation-policy@2",
    ...
    assert (
        projected["history"]["market_snapshots"][0]["market_snapshot_id"]
        == market.market_snapshot_id
    )
    assert [
        item["plan_evaluation_id"] for item in projected["history"]["evaluations"]
    ] == [evaluation.plan_evaluation_id, upgraded.plan_evaluation_id]
    assert [
        item["evaluation_policy_version"]
        for item in projected["history"]["evaluations"]
    ] == ["evaluation-policy@1", "evaluation-policy@2"]
    assert projected["history"]["evaluations"][0]["rules"][0]["operands_json"]
    assert (
        projected["history"]["research_runs"]
        and projected["history"]["data_snapshots"]
        and projected["history"]["artifact_manifests"]
    )
    root.close()


def test_connected_golden_journey_records_one_graph_on_one_data_root(
    tmp_path: Path,
) -> None:
    ...
    annotation_input = replace(
        annotation_draft(),
        data_snapshot_id=sync.snapshot_id,
        links=(AnnotationLink("ResearchRun", research.research_run_id, "resolved"),),
    )
    annotation = root.chart.create(
        AnnotationCommand("golden:annotation", None, 0, annotation_input)
    )
    content = PlanDraftContent(
        ...
        (
            ...
            PlanReference("Evidence", "golden:fixture", "unresolved_external"),
        ),
        sync.snapshot_id,
        "2026-07-11",
        "2026-10-11",
        ...
        (
            PlanRule(
                ...
                PlanCondition(
                    ...
                    "lte",
                    PlanConstant("decimal", "80", "CNY_per_share", "CNY"),
    ...
    draft = root.plans.create_draft(
        CreatePlanDraftCommand("golden:plan-draft", content)
    )
    plan = root.plans.confirm_draft(
        ConfirmPlanDraftCommand("golden:plan-confirm", draft.draft_id, 1, "activate")
    )
    ...
    market = root.market.build_market_snapshot(
    ...
    assert (
        manifest.producer_id == research.workflow_run_id
        and doctor["status"] == "passed"
    )
    assert (
        rebuilt.chart.get_history(annotation.annotation_id)[0].annotation_version_id
        == annotation.annotation_version_id
    )
    assert (
        rebuilt.plans.get_version(plan.plan_version_id).content.references[0].ref_id
        == research.research_run_id
    )
    assert (
        rebuilt.market.get_market_snapshot(market.market_snapshot_id).data_snapshot_id
        == sync.snapshot_id
    )
    assert (
        rebuilt.market.get_plan_evaluation(
            evaluation.plan_evaluation_id
        ).market_snapshot_id
        == market.market_snapshot_id
    )
    assert (
        workspace["history"]["research_runs"]
        and workspace["history"]["plans"]
        and workspace["history"]["evaluations"]
    )
    evidence = {
        "schema_version": "GoldenJourneyEvidence@1",
        "workflow_run_id": research.workflow_run_id,
        "original_workflow_run_id": original.workflow_run_id,
        "data_snapshot_id": sync.snapshot_id,
        "research_snapshot_id": original.research_snapshot_id,
        "research_run_id": research.research_run_id,
        "research_json_artifact_id": original.json_artifact_id,
        "research_html_artifact_id": original.html_artifact_id,
        "annotation_version_id": annotation.annotation_version_id,
        "plan_version_id": plan.plan_version_id,
        "market_snapshot_id": market.market_snapshot_id,
        "plan_evaluation_id": evaluation.plan_evaluation_id,
        "final_artifact_manifest_id": history.final_manifest_id,
        "reuse_reason_code": research.reason_code,
        "stale_by_days": research.stale_by_days,
        "dispositions": {
            "research": research.disposition.value,
            "annotation": "created",
            "plan": "created",
            "market": "created",
            "evaluation": "created",
...
def test_browser_plan_confirmation_preserves_user_fixture_source_and_old_history(
    tmp_path: Path,
) -> None:
    root = plan_root(tmp_path)
    draft = root.plans.create_draft(
        CreatePlanDraftCommand("workspace:draft", _content(root))
    )
    server = LocalChartWorkspaceServer(
        decision_workspace=root.workspace,
        chart_workspace=root.chart,
    ...
    headers = {
        ...
        "Content-Type": "application/json",
        "X-CSRF-Token": token,
        "X-Invocation-Id": "browser:confirm-plan",
    }
    confirmed = json.loads(
        urlopen(
            Request(
                base + "/api/plan-confirmations",
                data=body,
                method="POST",
                headers=headers,
            )
    ...
    revised_content = replace(
        _content(root),
        based_on_version_id=confirmed["plan_version_id"],
        rationale="用户补充了新的复核理由。",
    )
    revised_draft = root.plans.create_draft(
        CreatePlanDraftCommand("workspace:draft:v2", revised_content)
    )
    ...
    assert [item["plan_version_id"] for item in workspace["history"]["plans"]] == [
        confirmed["plan_version_id"],
        revised["plan_version_id"],
    ]
...
def test_script_shaped_annotation_and_path_inputs_cannot_execute_or_escape(
    ...
    with pytest.raises(HTTPError) as rejected:
        urlopen(
            Request(
                base + "/api/annotations", data=attack, method="POST", headers=headers
            )
        )
...
def test_secret_and_personal_paths_never_reach_dom_logs_or_artifacts(
    ...
    combined = (
        html + workspace + artifact_payloads + output.out.encode() + output.err.encode()
    )
    assert (
        marker.encode() not in combined
        and str(tmp_path).encode() not in html + workspace
    )
    request = Request(
