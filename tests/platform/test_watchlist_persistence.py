...
from trading_platform.application.bootstrap import open_platform_operations
...
def test_bootstrap_watchlist_replay_restart_and_doctor(tmp_path: Path) -> None:
    app = PlatformTaskFixture(tmp_path)
    assert app.health.inspect(HealthQuery()).capabilities[Capability.PERSISTENCE] is CapabilityStatus.AVAILABLE
    security = SecurityIdentity("security_yihua_a", "SZSE", "002897", "CNY", "2026-07-10")
    first = app.watchlist.add("inv-1", security)
    assert app.watchlist.add("inv-1", security) == first
    assert app.watchlist.add("inv-2", security) == first
    assert open_platform_operations(tmp_path).doctor()["status"] == "passed"
    app.close()
    reopened = PlatformTaskFixture(tmp_path)
    assert reopened.watchlist.list() == (first,)
    reopened.close()
...
def test_second_writer_future_ledger_and_identity_conflict_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(PersistenceError) as busy:
        store.watchlist.add("inv", SecurityIdentity("stable", "SZSE", "002897", "CNY", "2026-07-10"))
    ...
    with pytest.raises(PersistenceError) as conflict:
        store.watchlist.add("inv-2", SecurityIdentity("stable", "SZSE", "002897", "USD", "2026-07-10"))
...
def test_n_minus_one_upgrade_is_backup_first_preserves_date_precision_and_rolls_back_failure(tmp_path: Path) -> None:
    ...
    store.watchlist.add("inv", security)
    store.close()
    ...
    assert upgraded.watchlist.list()[0].security_id == "stable"
    upgraded.close()
    ...
    assert failed.watchlist.list()[0].security_id == "stable"
    failed.close()
