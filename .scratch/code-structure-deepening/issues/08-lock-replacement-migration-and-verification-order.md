...
6. **Release acceptance**：`python -m pytest -q -m release_acceptance tests/platform/test_acceptance_evidence.py` 必须0 skipped；随后运行 `python -m trading_platform.cli acceptance --data-root <fresh-root> --fixture-manifest tests/fixtures/platform_data/manifest.json --repo-root .`，该 canonical command 必须自行调用 production browser verifier 并冻结带 verifier/browser/CDP identity 的 typed evidence；若有当前 live qualification artifact则显式传入。acceptance suite ledger、51 criteria、command identities、browser/backup/legacy-replacement evidence和immutable manifest自校验全部通过；真实 provider不可用可保持精确 `external_blocked`，但不能伪造 qualified或把结构回归标为通过。
