...
function renderWorkspace(model){
  ...
  const positions=model.current_positions??[]
  document.querySelector("#position-status").textContent=model.security_relationship==="position"?"该标的存在当前仓位；以下数值来自所示冻结账户快照。":"当前账户快照显示无仓位；这不同于仓位数据缺失。"
  document.querySelector("#position-list").replaceChildren(...(positions.length?positions.map(position=>{const article=document.createElement("article");const heading=document.createElement("h3");heading.textContent=`${text(position.account_label)} · ${text(position.snapshot_as_of)}`;const facts=document.createElement("p");facts.textContent=`数量 ${text(position.quantity_decimal)} · 可用 ${text(position.available_decimal)} · 冻结 ${text(position.frozen_decimal)} · 成本基础 ${text(position.cost_price_decimal)} ${text(position.base_currency)} · 现金 ${text(position.cash_decimal)} · 对账 ${text(position.reconciliation_status)} · 新鲜度 ${text(position.freshness)}`;const limits=document.createElement("p");limits.textContent=`能力限制：${(position.limitations??[]).join("、")||"无"}`;article.append(heading,facts,limits);return article}):[Object.assign(document.createElement("p"),{textContent:model.security_relationship==="watchlist_not_held"?"本地账户快照确认无当前仓位。":"仓位数据缺失或账户尚未初始化；不能按零仓位解释。"})]))
  const plans=model.history?.plans??[]
  const planCards=plans.map(plan=>{const article=document.createElement("article");const heading=document.createElement("h3");heading.textContent=`计划 v${plan.version_no} · 已确认`;const copy=document.createElement("p");copy.textContent=`输入来源 ${text(plan.user_input_source)} · 冻结于 ${text(plan.created_at)} · 账户快照 ${text(plan.account_snapshot_id)}`;article.append(heading,copy);return article})
