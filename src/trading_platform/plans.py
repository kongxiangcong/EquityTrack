...
from trading_platform.domain.plans import (
    ...
    PlanDiffItem,
    PlanDraftContent,
    PlanValidationError as PlanError,
    TradePlanDraftView,
...
class PlanService:
    ...
    def get_account_operands(self, plan_version_id: str) -> dict[str, str]:
        return self.repository.get_account_operands(plan_version_id)
    ...
    def confirmation(self, draft_id: str) -> PlanConfirmationView:
        ...
        if draft.content.based_on_version_id:
            before = asdict(
                self.repository.get_version(draft.content.based_on_version_id).content
            )
            after = asdict(draft.content)
            diff = tuple(
                PlanDiffItem(key, before.get(key), after.get(key))
        ...
        sections = (
            PlanConfirmationSection(
                ...
                (
                    ...
                    ("data_snapshot_id", content.data_snapshot_id),
                    ("account_snapshot_id", content.account_snapshot_id),
                    ("horizon_start", content.horizon_start),
                    ("horizon_end", content.horizon_end),
            ...
            PlanConfirmationSection(
                ...
                (
                    ("rules", content.rules),
                    ("metric_catalog_version", content.metric_catalog_version),
                    ("evaluator_policy_version", content.evaluator_policy_version),
                ),
            ),
            PlanConfirmationSection(
                "risk_budget",
                (
                    ...
                    ("max_planned_notional", content.max_planned_notional),
                    ("max_planned_loss", content.max_planned_loss),
                    (
                        "portfolio_feasibility",
                        (
                            "exact_snapshot_context"
                            if content.account_snapshot_id
                            else "not_applicable_no_account_or_position"
                        ),
                    ),
                ),
            ),
            PlanConfirmationSection(
                "market_gates",
                (
                    ("market_gate_policy_version", content.market_gate_policy_version),
                    (
                        "market_gate_rules",
                        tuple(
                            rule
                            for rule in content.rules
                            if rule.rule_kind == "market_gate"
                        ),
                    ),
                ),
            ),
        )
        return PlanConfirmationView(
            draft_id,
            content,
            draft.content_hash,
            sections,
            diff,
            content.user_input_source,
