...
> 状态：设计已确认
> 实现状态：尚未按本文重构；当前代码和 `skills/SKILL.md` 仍描述既有运行时
> 后续入口：由用户另行手动执行 `to-spec`，再执行 `to-issue`
...
当前 `skills/SKILL.md` 在代码切换前仍描述既有运行时；不能提前改写成尚未实现的任务合同。
...
本轮只交付设计文档。后续 `to-spec` 与 `to-issue` 由用户手动触发；本轮不修改运行时代码，不访问或修改 `E:\trading-data\kong`。
...
- **设计已确认**：本文、领域词汇和 ADR 已收敛；当前状态。
- **合成通路已验证**：实现、合成闭环、迁移恢复、测试和旧路径清理全部通过，但真实 Provider 尚未验证。
